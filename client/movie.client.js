  ...
  try {
    for (let i = 1; i <= NUMBER_OF_PAGES; i++) {
      const moviesResponse = await fetch(
        appendApiKey(`${movieDbUrl}/movie/popular`).concat(`&page=${i}`),
        {
          method: 'GET',
      ...
      moviesResponse.json().then((data) => {
        data.results.map(async (movieObj) => {
          ...
          if (
            !words.some((profanity) =>
              details.title
                .toLowerCase()
                .includes(profanity.trim().toLowerCase())
            )
          ) {
...
export const fetchGenres = async () => {
  ...
  try {
    const genresResponse = await fetch(
      appendApiKey(`${movieDbUrl}/genre/movie/list`),
      {
        method: 'GET',
        headers: defaultHeaders,
      }
...
const fetchTmdbDetails = async (id) => {
  ...
  try {
    const response = await fetch(
      appendApiKey(`${movieDbUrl}/movie/${id}`).concat(
...
const fetchOmdbDetails = async (imdbId) => {
  ...
  try {
    const response = await fetch(
      `${omdbUrl}?apikey=${omdbApiKey}&i=${imdbId}`,
      {
        method: 'GET',
        headers: defaultHeaders,
      }
    ...
    const socialRatings =
      omdbResponseJson.Response === 'False' ? null : omdbResponseJson.Ratings;
...
const fetchMovieCast = async (id) => {
  ...
  try {
    const response = await fetch(
      appendApiKey(`${movieDbUrl}/movie/${id}/credits`),
      {
        method: 'GET',
        headers: defaultHeaders,
      }
...
export const fetchRegionMovies = async (region, limit) => {
  ...
  try {
    const response = await fetch(
      appendApiKey(`${movieDbUrl}/movie/popular`).concat(
...
export const fetchPopularShows = async () => {
  ...
  try {
    const response = await fetch(
      appendApiKey(`${episodateUrl}/most-popular?page=1`),
      {
        method: 'GET',
        headers: defaultHeaders,
      }
