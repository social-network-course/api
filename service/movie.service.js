...
import {
  RECOMMENDER_NO_OF_LOOKUP_USERS,
  RECOMMENDER_NO_OF_LOOKUP_USERS_RATINGS,
} from '../util/constants.js';

export const getGenres = async () => {
  const genres = await Genre.find({}, 'id name').sort({ name: 'asc' });

  return genres;
};

export const getStatuses = async () => {
  const statuses = await Status.find({}, 'id name').sort({ name: 'asc' });
...
export const getTopRatedMovies = async ({ page, limit, genre, status }) => {
  ...
  const movies = await buildAggregation(
    query,
    { vote_average: -1, title: 1 },
    limit,
    page
  );
  ...
  return {
    ...
    pages:
      movies[0].info.length > 0
        ? Math.ceil(Number(movies[0].info[0].total) / limit)
        : movies[0].info,
  };
};
...
export const getRecommendedMovies = async (
  ...
  if (activeUser.ratings.length > 0) {
    ...
    const normalizedActiveUserRatings = normalizeUserRatings(
      activeUser.genre_ratings.ratings,
      activeUser.genre_ratings.avgRatingAggregate.avgRating
    );

    otherUsers.forEach((otherUser) => {
      if (otherUser.ratings.length > 0) {
        const normalizedOtherUserRatings = normalizeUserRatings(
          otherUser.genre_ratings.ratings,
          otherUser.genre_ratings.avgRatingAggregate.avgRating
        );
        const pearsonCorrelation = getPearsonCorrelation(
          normalizedActiveUserRatings,
          normalizedOtherUserRatings
        );
    ...
    const topLookupUsers = pearsonCoefficients.slice(
      0,
      RECOMMENDER_NO_OF_LOOKUP_USERS
    );
    let movies = [];
    for (const topLookupUser of topLookupUsers) {
      const user = otherUsers.find(
        (user) => user.id === Number(topLookupUser.userId)
      );
      const topLookupUserRatings = user.ratings.slice(
        0,
        RECOMMENDER_NO_OF_LOOKUP_USERS_RATINGS
      );
      for (const rating of topLookupUserRatings) {
        if (
          !activeUser.ratings.find(
            (activeUserRating) => activeUserRating.movieId === rating.movieId
          )
        ) {
          ...
          if (
            movie &&
            !movies.find((recommendedMovie) => recommendedMovie.id === movie.id)
          ) {
            movies.push(movie);
          }
        }
...
export const getFeaturedMovies = async ({ limit }) => {
  const featuredMovies = await Movie.find(
    {},
    'id title backdrop_path overview genre_ids vote_average'
  )
...
export const getMoviesInTheaters = async ({ limit }) => {
  ...
  const todayMinusInterval = new Date(
    new Date(today).setDate(today.getDate() - interval)
  );

  const moviesInTheaters = await Movie.find(
    {
      release_date: {
...
export const getTopRevenueMovies = async ({ limit }) => {
  const topRevenueMovies = await Movie.find({}, 'id title revenue')
    .sort({ revenue: 'desc' })
    .limit(Number(limit));
...
export const getMostVisitedMovies = async ({ limit }) => {
  const mostVisitedMovies = await Movie.find(
    { visit_counter: { $gt: 0 } },
    'id title backdrop_path visit_counter'
  )
...
const getPearsonCorrelation = (
  ...
  for (let i = 0; i < normalizedActiveUserRatings.length; i++) {
    if (
      normalizedActiveUserRatings[i] === null ||
      normalizedOtherUserRatings[i] === null
    ) {
      continue;
    } else {
      pearsonNumerator +=
        normalizedActiveUserRatings[i] * normalizedOtherUserRatings[i];
      pearsonDenominatorActiveUser += Math.pow(
        normalizedActiveUserRatings[i],
        2
      );
  ...
  const pearsonDenominator = Math.sqrt(
    pearsonDenominatorActiveUser * pearsonDenominatorOtherUser
  );
